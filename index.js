...
app.get('/ci-cd', (req, res)=>{
  res.json({message: "After gokul fixing container information from jenkins again with proper build checking if mongo isdkjfd getting existed sldjlsjdor not and also pulled changes with gokul webhook with jenkins after webhook created skjdks this is from deployment fix after turninig off redeploy this is frpm deployment check if local host is removed or not"});
})
